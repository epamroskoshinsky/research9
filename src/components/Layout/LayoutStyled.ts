...
export const GridWrapperContent = styled( Grid )<{structure?:boolean, header?:boolean}>`
    padding: ${ ({header}) => ( header ? NAVIGATION_HEADER_HEIGHT : 0 ) }px 0 0 ${ ({structure}) => (structure ? NAVIGATION_SIDEBAR_WIDTH + NAVIGATION_STRUCTURE_WIDTH : NAVIGATION_SIDEBAR_WIDTH) }px;
`;
